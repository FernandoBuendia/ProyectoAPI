...
async def update_task(task_id: int, task_update: TaskUpdateModel, user: User = Depends(get_current_user)):

  task_to_update = Task.select().where((Task.id == task_id) & (Task.user_id == user.id)).first()
...
async def delete_task(task_id: int, user: User = Depends(get_current_user)):
  ...
  task_to_delete = Task.select().where((Task.id == task_id) & (Task.user_id == user.id)).first()
